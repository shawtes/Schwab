...
import { TopBar, MainView } from "@/components/TopBar";
import { CommandPalette } from "@/components/CommandPalette";
import { MLAutoTrader } from "@/components/MLAutoTrader";
...
export default function TradingPage() {
  ...
  const [currentView, setCurrentView] = useState<MainView>("trading");

  // Queries - Load initial data ONCE, then let WebSocket handle live updates
  const { data: historyData, isLoading: historyLoading, isRefetching: historyRefetching } = useQuery({
    queryKey: ["history", symbol, timeframe],
    queryFn: () => fetchHistory(symbol, timeframe),
    refetchInterval: false, // No polling - WebSocket handles live candles
  ...
  const { data: watchlistData, isLoading: watchlistLoading } = useQuery({
    ...
    refetchInterval: false, // No polling - WebSocket handles live quotes
    staleTime: Infinity // Initial load only, WebSocket handles updates
  });

  const { data: orderBookData, isLoading: orderBookLoading } = useQuery({
    queryKey: ["orderbook", symbol],
    queryFn: () => fetchOrderBook(symbol),
    refetchInterval: false, // No polling - WebSocket handles live order book
    staleTime: Infinity // Initial load only, WebSocket handles updates
  });
  ...
  // Get current price from watchlist for the selected symbol
  const watchlistCurrentPrice = watchlistData?.find(q => q.symbol === symbol)?.last;
    ...
    <div className="h-screen flex flex-col bg-background overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
              ...
              <div className="h-[60%] p-2">
                {historyLoading ? (
                  ...
                  <Chart 
                    data={historyData} 
                    symbol={symbol} 
                    currentPrice={currentPrice ?? watchlistCurrentPrice}
                    timeframe={timeframe}
        ...
        <div className="w-80 border-l border-border flex flex-col overflow-hidden">
          ...
          /* Full-Page Momentum Scanner */
          <div className="flex-1 overflow-hidden p-4">
            <MomentumScanner />
          </div>
        ) : currentView === "ml-trader" ? (
          /* Full-Page ML Auto Trader */
          <div className="flex-1 overflow-hidden p-4">
            <MLAutoTrader symbol={symbol} />
          </div>
        ) : null}
      </div>
